...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				System.out.println("�Vives con tus padres?" + 
			"Escribe 's' para si o 'n' para no");
				opcion = scanner.next();
	...
	public static String encontrarCuenta(Persona persona) {
		String cuenta;
		if (persona.getEdad() < 18 && persona.isEstudiante() &&
				persona.isCasaPadres()) {
			cuenta = "Cuenta Comfort";
			return cuenta;
		} else if (persona.getEdad() < 25 && persona.isEstudiante() &&
				!persona.isCasaPadres()) {
			cuenta = "Cuenta Vamos que t� puedes";
			return cuenta;
		} else if (persona.getEdad() > 18 && persona.getEdad() < 25 &&
				!persona.isEstudiante()
				&& persona.isCasaPadres()) {
			...
			cuenta = "No tenemos cuenta para ti," + 
		"�est�s seguro que has metido bien tus datos?";
			return cuenta;
		}
	}
	...
	NumeroNegativoException, TooLargeNumberException {
		if (number < 0) {
